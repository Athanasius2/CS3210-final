...
namespace cppfinal
{
	...
	void Animal::act()
	{
		...
		std::copy_if(adj.begin(), adj.end(), edible.begin(), [this](auto x) -> bool {if (x.second != nullptr) return this->is_dangerous(x.second->gettype()); return false; });
		...
		if (!(std::distance(it_dir, adj.end()) == 0))
		{
			if (!is_dead())
			{
				...
				if (!(std::distance(it_dir, adj.end()) == 0))
				{
					...
					std::vector<Actor> scanned{};
					std::vector<Actor> within_x{};
					const auto begin = get_environment()->get_actors().begin();
					const auto end = get_environment()->get_actors().end();
					auto result = std::copy_if(begin, end, within_x.begin(), [this](auto a) -> bool { //return list of Actors 
						int xt = this->get_pos().first;
						int xa = a->get_pos().first;
						return (std::abs(xt - xa) <= 3)
					});

					result = std::copy_if(begin, end, scanned.begin(), [this](auto a) -> bool { //return list of Actors 
					...
					*/
					...
					case HUNGRY:
						...
						if (!edible.empty())
						{
							std::sort(edible.begin(), edible.end(), [](auto x, auto y) -> bool { return x.second->get_energy() > y.second->get_energy(); });
							edible.begin()->second->eat();
							move(edible.begin()->first);
						}
						else
						{
							std::copy_if(scanned.begin(), scanned.end(), edible.begin(), [this](auto x) -> bool { return this->is_dangerous(x.second->gettype()); });

							if (!edible.empty())
							{
								//calculate the average direction of scanned food and move there.
								std::for_each(edible.begin(), edible.end(), [&movedir, this](auto x) {
					...
					case REPRODUCE:
						std::copy_if(adj.begin(), adj.end(), reproduce.begin(), [this](auto x) {return x.second->can_reproduce(this->gettype()); });
						if (!reproduce.empty())
						{
					...
					case HUNTED:
						std::copy_if(adj.begin(), adj.end(), hunters.begin(), [this](auto x) {return x.second->is_dangerous(this->gettype()); });
						if (!hunters.empty())
						{
